  ...
  const handleSubmit = (event) => {
    ...
    if (film.title.length == 0) {
      ...
      if (props.filmToEdit) {
        ...
        props.editFilm(film);
        // The state is to tell not to reload from server, later it will be removed
        navigate('/', {state: {reloadFromServer: false}});  
      } else {
        props.addFilm(film);
        // The state is to tell not to reload from server, later it will be removed
        navigate('/', {state: {reloadFromServer: false}});
  ...
  return (
    ...
    <Form onSubmit={handleSubmit}>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control type="number" min={0} max={5} step={1} value={rating} onChange={event => setRating(event.target.value)}/>
      </Form.Group>
