...
const purifyFilm = film => Object.assign({}, film, {title: DOMPurify.sanitize(film.title)});

const purifyFilms = filmList => filmList.map(e => purifyFilm(e));
...
app.get('/api/films', 
  (req, res) => {
    ...
    filmDao.listFilms(req.query.filter)
      .then(films => res.json(purifyFilms(films)))
...
app.post('/api/films',
  ...
  async (req, res) => {
    ...
    try {
      // It would be better to sanitize the film properties BEFORE inserting them in the DB
...
app.put('/api/films/:id/favorite',
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res.json(purifyFilm(result)); 
    } catch (err) {
